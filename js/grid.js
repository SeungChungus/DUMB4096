        ...
        for (var i = 0; i < xarr.length; i++) {
          ...
          if (tmpx >= 0 && tmpx < this.size && tmpy >= 0 && tmpy < this.size) {
            if (
              ...
              this.cells[tmpx][tmpy].value === content
            ) {
              return {
                x: x,
                y: y,
...
Grid.prototype.maxCellMove = function (director) {
  ...
  var newMaxCell = {
    x: maxCell.x + directVector.x,
    y: maxCell.y + directVector.y,
    value: maxCell.value,
  };
  if (this.withinBounds(newMaxCell)) {
    ...
    if (newMaxCell.x == maxCell.x) {
      if (
        (maxCell.y == 3 && newMaxCell.y <= 2) ||
        (newMaxCell.y >= 1 && maxCell.y == 0)
      ) {
        return true;
      }
    } else {
      if (
        (maxCell.x == 3 && newMaxCell.x <= 2) ||
        (newMaxCell.x >= 1 && maxCell.x == 0)
      ) {
        return true;
      }
    }
