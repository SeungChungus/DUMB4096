...
GameManager.prototype.canMergeAlmostMax = function () {
  ...
  for (var i = 0; i < cells.length; i++) {
    ...
    for (var j = 0; j < vector.length; j++) {
      var tmpcell = {
        x: cells[i].x + vector[j].x,
        y: cells[i].y + vector[j].y,
      };
      if (
        this.grid.withinBounds(tmpcell) &&
        this.grid.cells[tmpcell.x][tmpcell.y] &&
        this.grid.cells[tmpcell.x][tmpcell.y].value === cells[i].value
      ) {
